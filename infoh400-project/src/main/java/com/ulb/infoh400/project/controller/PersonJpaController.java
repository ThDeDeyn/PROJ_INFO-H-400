 ...
 */
package com.ulb.infoh400.project.controller;
...
public class PersonJpaController implements Serializable {
    ...
    public Person findDuplicate(Person p){
        ...
        EntityManager em = getEntityManager(); 
        List<Person> res = em.createNamedQuery("Person.findDuplicate").setParameter("firstname", p.getFirstname())
                                                        .setParameter("familyname", p.getFamilyname())
                                                        .setParameter("dateofbirth", p.getDateofbirth()).getResultList();
